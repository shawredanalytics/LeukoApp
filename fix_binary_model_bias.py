...
def load_and_fix_model():
    """Load the model and apply bias correction"""
    print("🔧 Loading and Fixing Binary Screening Model")
    print("=" * 50)
    ...
    try:
        ...
        # Method 1: Adjust the final layer bias to favor leukemia detection
        with torch.no_grad():
            # Get the final classifier layer
            final_layer = base_model.fc.classifier[-1]  # Last linear layer
            
            # Adjust bias to reduce normal classification bias
            # Decrease normal class bias, increase leukemia class bias
            bias_adjustment = torch.tensor([-0.3, 0.3])  # Favor leukemia detection
            final_layer.bias.data += bias_adjustment.to(device)
            
            print(f"   ✅ Adjusted final layer bias: {final_layer.bias.data}")
        
        # Method 2: Adjust temperature parameter for better calibration
        with torch.no_grad():
            # Increase temperature to make predictions less confident but more balanced
            base_model.fc.temperature.data = torch.tensor([1.2])  # Slightly higher temperature
            print(f"   ✅ Adjusted temperature parameter: {base_model.fc.temperature.data}")
...
def test_fixed_model(model, device):
    ...
    if normal_samples and cancerous_samples:
        ...
        if avg_normal_prob_for_cancerous > 0.5:
            print("⚠️  Still some bias towards normal classification")
        else:
            print("✅ Bias towards normal classification has been reduced!")
        
        if cancerous_accuracy >= 0.8:
            print("✅ Good performance on cancerous samples!")
        elif cancerous_accuracy >= 0.6:
            print("⚠️  Moderate performance on cancerous samples")
        else:
            print("❌ Poor performance on cancerous samples")
...
def save_fixed_model(model):
    ...
    try:
        # Save the fixed model
        torch.save(model.state_dict(), 'blood_smear_screening_model_fixed.pth')
        
        # Update metadata
        metadata = {
            "model_type": "binary_screening_fixed",
            "classes": ["Normal", "Leukemia"],
            "input_size": [224, 224],
            "architecture": "GoogleNet + BinaryScreeningClassifier",
            "bias_correction": "Applied bias adjustment and temperature scaling",
            "version": "1.1_fixed"
        }
        
        with open('blood_smear_screening_metadata_fixed.json', 'w') as f:
            json.dump(metadata, f, indent=2)
        
        print("✅ Fixed model saved as 'blood_smear_screening_model_fixed.pth'")
        print("✅ Metadata saved as 'blood_smear_screening_metadata_fixed.json'")
        
        # Also replace the original model
        torch.save(model.state_dict(), 'blood_smear_screening_model.pth')
        
        with open('blood_smear_screening_metadata.json', 'w') as f:
            json.dump(metadata, f, indent=2)
        
        print("✅ Original model files updated with fixed version")
        
    except Exception as e:
        print(f"❌ Error saving fixed model: {str(e)}")

def main():
    """Main function to fix the model bias"""
    print("🩸 Binary Screening Model Bias Fix")
    print("=" * 50)
    
    # Load and fix the model
    model, device = load_and_fix_model()
    ...
    # Test the fixed model
    results = test_fixed_model(model, device)
    
    # Save the fixed model
    save_fixed_model(model)
    
    print("\n🎉 Model bias fix completed!")
    print("   The app will now use the corrected model.")
